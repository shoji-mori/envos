...
|---|---|---|
| **グループ A: 結果の正しさに関わる問題** | | |
| A-1 | P1-1 | **対応済み**(⚠物理変更・G3b再生成) |
| A-2 | P1-2 | **対応済み** |
| A-3〜A-6 | P1-11 | **対応済み** |
| A-7 | P0-2 | **対応済み**(develop に統合済み) |
| A-8 | P1-9 | **対応済み**(⚠物理変更・G4追加) |
| A-9 | P1-8(根治 P2-A) | **対応済み**(P2-Aで根治) |
| A-10 | P0-2 | **対応済み**(develop に統合済み) |
| **グループ B: 確実にクラッシュする・呼べば必ず壊れるもの** | | |
| B-11 | P1-3(根治 P2-A) | **対応済み**(P2-Aで根治) |
| B-12, B-13 | P1-4 | **対応済み** |
| B-14〜B-16 | P1-10 | **対応済み** |
| B-17, B-18 | P1-12 | **対応済み** |
| B-19〜B-22 | P2-D | **対応済み** |
| B-23〜B-25 | P1-13 | **対応済み** |
| B-26 | P1-6(関数削除でクローズ) | **対応済み** |
| B-27 | P1-12(関数ごと削除) | **対応済み** |
| B-28 | P1-8 | **対応済み** |
| B-29, B-30 | P1-5 | **対応済み** |
| B-31〜B-33 | P1-17 | **対応済み** |
| B-34〜B-37 | P1-16 | **対応済み** |
| B-38, B-39 | P1-18 | **対応済み** |
| B-40 | P1-19 | **対応済み** |
| B-41 | P1-20 | **対応済み** |
| **グループ C: 特定の使い方で壊れる・サイレントに誤るもの** | | |
| C-42, C-43 | P1-2 | **対応済み** |
| C-44 | 先送り(D2) | 未着手 |
| C-45〜C-47 | P1-11 | **対応済み** |
| C-48 | P1-9 | **対応済み** |
| C-49 | 据え置き(D10) | **注記済み**(docstring) |
| C-50, C-51 | P1-7 | **対応済み** |
| C-52 | 据え置き(物理コード非ゴール原則) | 未着手 |
| C-53 | P1-8(NaN警告のみ) | **対応済み** |
| C-54, C-55 | P1-14 | **対応済み** |
| C-56 | P1-15 | **対応済み** |
| C-57, C-58 | P1-10 | **対応済み**(C-57はNotImplementedError化) |
| C-59〜C-61 | P1-11 | **対応済み** |
...
| D-70 | P0-3〜P0-5 | **対応済み** |
| **追補(2026-06-12 発見)** | | |
| 追補-71 | P1-11 | **対応済み**(save経路はP2-Dで復活) |
| 追補-72 | P1-4 | **対応済み** |
| 追補-73 | P2-C | **対応済み**(関数ごと削除) |
| 追補-74 | P1-12 | **対応済み** |
| 追補-75 | P1-2 | **対応済み** |
| 追補-76 | P1-12 | **対応済み** |
| 追補-77 | P1-11 | **対応済み** |
...
78. **`envos/plot_tools/physical_structure.py` — `plot_velocity_midplane_profile(midplane_average=False)` で
    速度配列が形状 `(nr, nphi)` のまま `gm`(形状 `(nr,)`)と乗算され `(nr, nr)` にブロードキャストされる。**
    matplotlib はエラーを出さず nr 本の線を描画する(サイレントな誤描画。Cランク相当。
    P1-16〜18 実施時に発見。修正は別タスク)。

79. **`envos/obs/data.py` — `PVmap.__post_init__(self, xrad, freq0)` の引数順が InitVar 宣言順
    (`freq0`, `xrad`)と逆。** dataclass は宣言順で InitVar を渡すため、`PVmap(freq0=...)` と
    すると freq0 の値が xrad として解釈され `xau` にスカラーが書き込まれる(Bランク相当。
    P2-D 実施時に発見。fits_io 側は当該コンストラクタ経路を回避済みだが、本体の修正は別タスク)。
